...
\end{itemize}
In our case the set of documents is the set of ICD-10 codes, each code representing a document. For each patient case, there is several sentences. A sentence forms one query, where the words in the sentence are classified as terms. By using BM25F, we get a ranked list of best matching ICD-10 codes to the sentence, and thereby an indication of what is wrong with the patient. 
